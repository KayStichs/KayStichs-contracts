...
//
// Spenders are stored as `bool` rather than as `Option<()>` style keys so the
// same key can be flipped back to `true` cheaply, and so the `get(key)` cost is
// constant rather than scoped to lookup-result. Adding/removing spenders is
// *additive* in v1.0; a `remove_approved_spender` path is tracked in
// `ROADMAP.md` under the *Next* bucket.
